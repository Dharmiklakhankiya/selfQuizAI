...
const TakeTest = () => {
  ...
  if (loading) return <div className="text-center text-cyan-300 font-mono text-2xl p-10">⚙️ Loading Quiz...</div>;
  if (error) return <div className="max-w-4xl mx-auto p-4 my-8 bg-red-500/20 text-red-300 rounded-lg border border-red-500/50">{error}</div>;
  if (!quiz) return <div className="text-center p-10 text-pink-300">No quiz data found.</div>;
  ...
  if (isFinished) {
    ...
    return (
      <div className="max-w-3xl mx-auto mt-10 p-8 bg-black/40 rounded-xl border border-indigo-500/30 backdrop-blur-md text-center text-cyan-100 shadow-[0_0_30px_#6366f1]">
        ...
        <p className="text-xl mb-2">Score: <span className="text-green-400 font-bold">{finalScore}</span> / {totalQuestions}</p>
        <p className="text-sm text-gray-300 mb-6">⏱ Time taken: <span className="text-cyan-300">{formatTime(elapsedTime)}</span></p>
        ...
        <button onClick={() => navigate('/history')} className="mt-8 w-full bg-indigo-600 hover:bg-indigo-700 text-white py-3 rounded-lg font-bold transition-all duration-300">
          🔙 Return to History
        </button>
      </div>
    );
  ...
  return (
    <div className="max-w-3xl mx-auto mt-10 p-8 bg-[#0f172a]/80 rounded-xl border border-indigo-500/20 text-white shadow-[0_0_30px_#6366f1] backdrop-blur-md">
      ...
      <div className="w-full bg-gray-700 rounded-full h-2.5 mb-6">
        <div className="bg-indigo-500 h-2.5 rounded-full transition-all duration-300 ease-in-out"
      ...
      <div className="mt-8 flex justify-between">
        ...
        {currentQuestionIndex < totalQuestions - 1 ? (
          <button onClick={handleNext} className="bg-indigo-600 text-white px-6 py-2 rounded-md font-semibold hover:bg-indigo-700">
            Next →
          </button>
        ) : (
          <button onClick={handleFinish} className="bg-green-600 text-white px-6 py-2 rounded-md font-semibold hover:bg-green-700">
            🚀 Finish Quiz
          </button>
        )}
      </div>
